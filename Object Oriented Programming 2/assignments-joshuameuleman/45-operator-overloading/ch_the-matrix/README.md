...
Implement a `Matrix` class that supports several overloaded operators to allow the following main program (`main.cpp`) to compile and function correctly **without any modifications**.

You will write the implementation of `Matrix.h` and `Matrix.cpp`, based on the provided main program and specifications below.

## Provided Code: `main.cpp`
...
* Implement `operator+` and `operator-` as member or friend functions.
...
* Support expressions like `matrix * scalar` and `scalar * matrix`.
* Use at least one **non-member** overload (to support `scalar * matrix`).
* Call the private `scalarMultiplication(int)` method from the operator overload.

![IMAGE](./img/Afbeelding3.png)

### Matrix Multiplication

* Implement `operator*` for matrix × matrix.
* Standard matrix multiplication rules apply: `A(m x n) * B(n x p) = C(m x p)`

![IMAGE](./img/Afbeelding4.png)

### Other Guidelines

* Pass objects by reference as much as possible.
* Mark `getElement`, `getString`, and `scalarMultiplication` as `const`.
* Use dynamic memory (allocate and deallocate with care in constructors, copy constructor, destructor).
* Use at least **two different styles** of operator overloading: member and non-member (friend).
