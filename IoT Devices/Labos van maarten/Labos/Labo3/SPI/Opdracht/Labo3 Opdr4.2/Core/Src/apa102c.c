...
// Verstuur de data over SPI1. Let op: zorg dat SPI1 reeds ge�nitialiseerd is.
void UpdateAPA102CLeds(SPI_HandleTypeDef* hspi, APA102C led[])
{
	...
	// Alle data verzenden via blokkerende SPI (timeout 100ms).
	HAL_SPI_Transmit(hspi, source, (4 + NUMBER_OF_APA102C_LEDS * 4 + 4), 100);
}
