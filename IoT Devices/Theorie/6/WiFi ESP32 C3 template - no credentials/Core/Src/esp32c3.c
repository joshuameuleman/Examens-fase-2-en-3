...
void HttpRequest(char host[], char path[], uint16_t port, enum HttpVerb method, char data[])
{
	...
	while(PopStringFromCircularBuffer(&circularBuffer, text) != READ_STRING_SUCCEEDED);		// Echo van "AT-CPISTART" weggooien.
	while(PopStringFromCircularBuffer(&circularBuffer, text) != READ_STRING_SUCCEEDED);		// Antwoord van "AT-CIPSTART" opvragen.
