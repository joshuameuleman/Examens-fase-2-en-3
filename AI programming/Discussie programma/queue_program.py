"""
Queue demo (NumPy-generated values)

Genereert waarden met NumPy en gebruikt een deque voor FIFO gedrag.
"""
...
def build_random_queue(min_items: int = 5, max_items: int = 12, count: Optional[int] = None) -> Deque[int]:
    q: Deque[int] = deque()
...
def drain_queue(q: Deque[int]) -> List[int]:
    result: List[int] = []
    while q:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Queue demo: bouw en drain een queue met integers (NumPy-generated)")
...
if __name__ == "__main__":
    ...
    queue = build_random_queue(min_items=args.min, max_items=args.max, count=args.count)
    print(f"Aantal elementen in queue (voor): {len(queue)}")

    items = drain_queue(queue)

    print("Elementen uit de queue (FIFO):")
    for i, value in enumerate(items):
        print(f"{i+1}. {value}")

    print(f"Aantal elementen in queue (na): {len(queue)}")
